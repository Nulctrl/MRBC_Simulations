...
QDs=[0, 28e-4, 14e-3, 7e-2, 35e-2]
Ds=[1e-1, 2e-1, 5e-1, 1.0, 2.0]
Names1=['0','28e-4','14e-3','7e-2','35e-2']
...
for i in range(len(QDs)):
    ...
    # Parameters
        ...
        d['g'] += dh * z # Add linear background
        m.fill_random('g', seed=41, distribution='normal', scale=1e-3) # Random noise
        m['g'] *= z * (Lz - z) # Damp noise at walls
        m['g'] += -Md * z # Add linear background

        # Analysis
        snapshot_name = f'snapshots {Names1[i]}, DH={Names2[j]}'
        snapshots = solver.evaluator.add_file_handler(snapshot_name, sim_dt=0.5, max_writes=50)
