...
wind = dist.Field(name='wind', bases=zbasis)
wind['g'] = 0.22*z
gamma = 0.5
...
problem.add_equation("dt(u) - nu*div(grad_u) + grad(p) + lift(tau_u2) = - u@grad(u)+ B*ez - gamma*(u_x - wind)*ex")
...
snapshots.add_task(integ1(m-d+Vaisala*ncc), name='Saturation')
